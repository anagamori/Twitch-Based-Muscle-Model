...
Af = length(FR);
meanForce_1 = length(FR); %mean force
meanForce_2 = length(FR); %mean force
...
beta = 2*rand(1);
for j = 1:100
    beta_1 = beta + 0.8./2.^(j-2);
    beta_2 = beta - 0.8./2.^(j-2);
    for i = 1:length(FR)
        %----------------------------------------------------------------------
        ...
        T2_temp = (RT(testingUnit)+RT(testingUnit)/4*f_env)/Lce; % half-relaxation time
        T2 = T2_temp/1.68;
        t_twitch = 0:1/Fs:5;
        ...
        meanForce_1(i) = mean(Force_1(3*Fs:4*Fs)); %mean force
        meanForce_2(i) = mean(Force_2(3*Fs:4*Fs)); %mean force
    end
