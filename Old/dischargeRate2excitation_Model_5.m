...
cd('/Users/akiranagamori/Documents/GitHub/Twitch-Based-Muscle-Model/Model Parameters/Model_5_constantT2T');
...
g_e = (parameterMN.I_max- parameterMN.I_th)./(1-modelParameter.U_th_new); 

f_t = 1.2;

Ur_t = 0.2;
index_t = find(U_th_new>=Ur_t);
f_k_e = fit([Ur_1 Ur_t]',[20 1]','poly1');
coeffs_f_k_e = coeffvalues(f_k_e);
lamda = coeffs_f_k_e(1)*U_th_new+coeffs_f_k_e(2);

I_half = (parameterMN.I_max-parameterMN.I_th);

k_e = (f_t*I_half-parameterMN.I_th+lamda.*(parameterMN.I_max-f_t*I_half))./(lamda.*(1-modelParameter.U_th_new));
U_th_t = (k_e-(parameterMN.I_max-f_t*I_half))./k_e;

%% 
cd('/Users/akiranagamori/Documents/GitHub/Twitch-Based-Muscle-Model/Code for Figures')
%% Discharge rate of motor unit
U_vec = 0:0.001:1;
I_mat = zeros(N_MU,length(U_vec));

for i = 1:length(U_vec)
    I = zeros(N_MU,1);
     
%    DR_temp = MDR + lamda.*k_e.*(U_vec(i)-U_th_new);
    I_temp_1 = parameterMN.I_th + lamda.*k_e.*(U_vec(i)-U_th_new);
    index_1 = find(U_vec(i) <= U_th_t);
    I(index_1) = I_temp_1(index_1);
    I_temp_2 = parameterMN.I_max-k_e*(1-U_vec(i));
    index_2 = find(U_vec(i) > U_th_t);
    I(index_2) = I_temp_2(index_2);
%     for n = 1:length(index_saturation)
%         index = index_saturation(n);
%         if U_vec(i) <= U_th_t(index)
%             I(index) = parameterMN.I_th(index) + lamda(index).*k_e(index).*(U_vec(i)-U_th_new(index));
%         else
%             I(index) = parameterMN.I_max(index)-k_e(index)*(1-U_vec(i));
%         end
%     end
    
    I_temp_3 = g_e.*(U_vec(i)-U_th_new)+parameterMN.I_th;  
...
index_plot = 200; %[1 10 20 40 100 210 250 279 282 300];
figure(1)
plot(U_vec*100,I_mat(index_plot,:),'b','LineWidth',1)
hold on 
plot([U_th_new(index_plot)*100 U_th_new(index_plot)*100],[-2 10],'k--','LineWidth',1)
plot([0 100],[parameterMN.I_max(index_plot) parameterMN.I_max(index_plot)],'k--','LineWidth',1)
plot([0 100],[parameterMN.I_th(index_plot) parameterMN.I_th(index_plot)],'k--','LineWidth',1)
