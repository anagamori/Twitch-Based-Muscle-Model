...
TendonVelocity = zeros(1,length(time));
TendonLength = zeros(1,length(time));
TendonAcceleration = zeros(1,length(time));
TendonLength(1) = L_se*L0T/100;
...
for t = 1:length(time)
    ...
    TendonAcceleration(t+1) = (k_ex*L_ex-F_se(t))/mass_ex;
    TendonVelocity(t+1) = TendonAcceleration(t+1)/Fs + TendonVelocity(t);
    TendonLength(t+1) = TendonVelocity(t+1)/Fs + TendonLength(t);
    
    L_se = TendonLength(t+1)/L0T;
    
    L_ex = Lmt - L_se*L0T - L_ce*L0*cos(alpha);
    %L_ex_dot = 
