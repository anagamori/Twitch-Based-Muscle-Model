...
cd(code_folder)
index_MU_1 = index_MU; 
CT_temp_1 = CT_vec(index_MU_1);
FR_half_1 = FR_half';

cd(model_parameter_folder)
load('FR_half_2')
load('CT_vec_2')
load('modelParameter_2')
cd(code_folder)
FR_half_2 = FR_half';
PDR_temp_2 = 2*FR_half';
index_temp = find(CT_vec<90);
r = randperm(length(index_temp),200);
index_MU_2 = sort(r)+4; 
CT_temp_2 = CT_vec(index_MU_2);

%[CT_temp_all,index_MU] = sort([CT_temp_1 CT_temp_2],'descend');
CT_temp = [CT_temp_1 CT_temp_2];
index_MU = [1:135 [1:126]+200 136:200 [127:200]+200];
CT_vec = CT_temp(index_MU);
cd(model_parameter_folder)
save('index_MU_2','index_MU_2')
save('CT_vec_new','CT_vec')
cd(code_folder)
...
modelParameter.tendonInitialLength = 27.5; %23.0; %24.1; % [cm]
...
modelParameter.N_MU = 400; % number of motor units
...
FR_half_temp = [FR_half_1(index_MU_1);FR_half_2(index_MU_2)];
modelParameter.FR_half = FR_half_temp(index_MU);
...
%% Fractional PSCA
F_pcsa_slow = 0.7; % fractional PSCA of slow-twitch motor units (0-1)
...
modelParameter.index_slow = 135+126;
%% Model parameters for activation-frequency relationship
cd(model_parameter_folder )
load('pool_parameter_matrix')
parameter_Matrix_temp_1 = parameter_Matrix(index_MU_1,:);
load('pool_parameter_matrix_2')
parameter_Matrix_temp_2 = parameter_Matrix(index_MU_2,:);
cd(code_folder)
parameter_Matrix_temp_all = [parameter_Matrix_temp_1;parameter_Matrix_temp_2];
modelParameter.parameterMatrix = parameter_Matrix_temp_all(index_MU,:);
