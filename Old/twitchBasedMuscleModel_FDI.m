...
%
%--------------------------------------------------------------------------
function output = twitchBasedMuscleModel_FDI(amplitude)
    ...
    if t > 1
        ...
        for j = 1:length(index)
            ...
            if ~any(spike_train(index_MU,:)) % initial time
                ...
                if f_env(index_MU) < 0.5
                    alpha = 1;
                else
                    alpha = a1_twitch*exp(-((f_env(index_MU)-b1_twitch)/c1_twitch)^2) + a2_twitch*exp(-((f_env(index_MU)-b2_twitch)/c2_twitch)^2);
                end
                [twitch,~,~] = twitch_function(Af,Lce,CT(index_MU),RT(index_MU),Fs,index_MU);
                force_temp = conv(spike_train_temp,PTi(index_MU)*twitch*alpha*twitch2tetanus_ratio);
...
%%
    function [twitch,T1,T2_temp] = twitch_function(Af,Lce,CT,RT,Fs,nMU)
        a1 = 9.459;
        b1 = 167.1;
        c1 = 30.58;
        a2 = 0.2205;
        b2 = 204.2;
        c2 = 550.1;
        a = a1*exp(-((nMU-b1)/c1)^2) + a2*exp(-((nMU-b2)/c2)^2);
        b = a1*exp(-((nMU-b1)/c1)^2) + a2*exp(-((nMU-b2)/c2)^2);
        T1 = CT*Lce^2+CT*Af*a;
