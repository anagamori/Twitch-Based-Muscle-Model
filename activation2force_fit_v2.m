...
for n = 1:N_MU
    if n <= index_slow
        FR_half(n) = 1/(CT(n)*1.3); %slow-twitch fibers
    else
...
%FR = 3*FR_half(testingUnit);
FR = [2 FR_half/2:5:3*FR_half(testingUnit) 3*FR_half(testingUnit)]; % testing firing rates
...
if testingUnit <= index_slow
    twitch2tetanus_ratio = 0.22*a_t2t; %slow-twitch fibers
else
    twitch2tetanus_ratio = 0.4*a_t2t; %fast-twitch fibers
end

%--------------------------------------------------------------------------
% Coefficient for non-linear force scaling 
a = 2.5; % unit 1: 2.2
b = 0.3; % unit 1:0.3
...
for i = 1:length(FR)
    ...
    spike = zeros(1,length(spikeTrain));
    Force = zeros(1,length(spikeTrain));
    Force_vec = zeros(1,length(spikeTrain));
    %----------------------------------------------------------------------
    ...
    for t = 1:length(f_env_2)
        ...
        if testingUnit <= index_slow
            ...
            Af_temp = Af_fast_function(f_eff,Lce,0.96);
        end
        Af_vec(t) = Af_temp;
        %==================================================================
        % Testing a new model
        ...
        if spikeTrain(t) == 1
            ...
            alpha = Force_vec(t-1)/(Force_vec(t-1)+0.1);
            T1 = CT(testingUnit)*Lce^2+CT(testingUnit)/4*alpha; % contraction time
            T2_temp = (RT(testingUnit)+RT(testingUnit)*1*alpha)/Lce; % half-relaxation time
            T2 = T2_temp/1.68;
            t_twitch = 0:1/Fs:5;
            ...
            %alpha = -0.2*Force(t)+1;
            %alpha = exp(log(0.3)*Force(t));
            twitch = twitch2tetanus_ratio*twitch;
            Force_temp = conv(spike,twitch);
            Force_temp = Force_temp(1:length(spikeTrain));
            Force = Force_temp + Force;           
        ...
        if testingUnit > index_slow
            Force_vec_temp = Force(t)*S;
            Force_vec(t) = Force_vec_temp.^a./(Force_vec_temp.^a+b^2);
        else
            Force_vec(t) = Force(t).^a./(Force(t).^a+b^2);
        end    
        if Force_vec(t) < 0.05
            a_S = a_S1;
        else
    ...
    % Plot output
    figure(1)
    plot(time,Force_vec) % new model
    hold on
    plot(time,Af_vec) % Tsianos et al. (2012)
    
    %----------------------------------------------------------------------
    % Calculate output variables
    meanForce(i) = mean(Force_vec(3*Fs:4*Fs)); %mean force
    P2PForce(i) = max(Force_vec(3*Fs:4*Fs))-min(Force_vec(3*Fs:4*Fs)); %peak-to-peak amplitude in force
