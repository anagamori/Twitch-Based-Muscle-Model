...
[Data] = MUModel_test(parameter,1,0,MU_type,1);
Af = Data{2,10};
FR_half = Data{2,6};
t2t = Data{2,5};
p2p = Data{2,13};
...
%%
figure()
plot(freq,Af','LineWidth',2,'color','b')
%xlim([0 100])
xlabel('Frequency (Hz)')
ylabel('Fusion (%)')
%xlim([0 3])
...
%%
figure()
plot(freq,fusion'*100,'LineWidth',2,'color','b')
%xlim([0 100])
xlabel('Frequency (Hz)')
ylabel('Fusion (%)')
%xlim([0 3])
...
%%
figure()
plot(freq,(1-p2p./p2p(1))*100,'LineWidth',1,'color','b')
hold on 
