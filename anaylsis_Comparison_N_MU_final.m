...
for i = 1:2
    ...
    shadedErrorBar(mean(mean_Force./MVC.*100),mean(std_Force./MVC.*100),std(std_Force./MVC.*100),'lineprops',{'color',color_code,'LineWidth',2,'markerfacecolor',color_code});
    hold on
    ...
    shadedErrorBar(mean(mean_Force./MVC.*100),mean(cov_Force),std(cov_Force),'lineprops',{'color',color_code,'LineWidth',2,'markerfacecolor',color_code});
    hold on
...
legend('Default','RP = 100','Ur = 0.8','Jones et al. 2002','location','northwest')
yticks(0:0.2:1.4)
xlim([0 100])
set(gca,'TickDir','out');
set(gca,'box','off')
...
temp = mean(mean_Force./MVC.*100);
temp2 = mean(std_Force./MVC.*100);
