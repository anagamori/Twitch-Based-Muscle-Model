...
%%
condition = 'Model_onion_skin_N_100_Ur_80_v2';
...
for k = 1:length(trial_vec)
    ...
    for i = 1:nTrial
        ...
        Force = Force_temp(7*Fs+2:end);
        window = 4*Fs;
        bp = [1:window:length(Force)];
        Force_dt = detrend(Force,2,bp);
...
%%
figure(3)
errorbar(amp_vec,mean(cov_Force_dt),std(cov_Force_dt),'LineWidth',2,'Color','b');
xlabel('Activation Level/Mean Force (%)','FontSize',14)
ylabel('CoV (%)','FontSize',14)
hold on 
errorbar(mean(mean_Force)./mean_mean_Force(end),mean(cov_Force_dt),std(cov_Force_dt),'LineWidth',2,'Color','r');
