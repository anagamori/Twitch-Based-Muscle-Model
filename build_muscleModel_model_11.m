...
model_parameter_folder =  '/Users/akiranagamori/Documents/Github/Twitch-Based-Muscle-Model/Model Parameters/Model_11';
...
% for i = 1:200
%     loc = find(PDR_temp==modelParameter.PDR(i));
%     %[~,loc] = min(abs(PDR_temp-modelParameter.PDR(i)));
%     idx_temp = [];
%     for j = 1:length(idx_all)
%         idx_temp = [idx_temp find(loc==idx_all(j))];
%     end
%     loc(idx_temp) = [];
%     idx_all(i) = loc(1);
% end
% index_MU = sort(idx_all); 
% idx_1 = find(PDR_temp>70);
% idx_2 = find(PDR_temp>=60&PDR_temp<70);
% r_2 = randperm(length(idx_2),20);
% idx_3 = find(PDR_temp>=50&PDR_temp<60);
% r_3 = randperm(length(idx_3),30);
% idx_4 = find(PDR_temp>=40&PDR_temp<50);
% r_4 = randperm(length(idx_4),30);
% idx_all = [idx_1;idx_2(r_2);idx_3(r_3);idx_4(r_4)];
% PDR_temp(idx_all) = [];
r =  randperm(275,200);
index_MU = [1:300/100:300 300:-3:2];
%index_MU(idx_all) = [];
index_MU = sort(r+5); 
%% Peak tetanic force
...
%% Fractional PSCA
F_pcsa_slow = 0.6; % fractional PSCA of slow-twitch motor units (0-1)
...
figure(1)
histogram(modelParameter.MDR)

figure(2)
histogram(modelParameter.PDR)

figure(3)
histogram(CT_vec(index_MU))
