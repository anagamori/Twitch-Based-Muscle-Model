...
for trialN = 196 %180:189
    ...
    cd(data_folder)
    load(['Data_' num2str(trialN) '_' num2str(1)])
    cd(code_folder)
    param_Matrix_1 = Data{2,12};
    error_1 = Data{2,8};
    cd(data_folder)
    load(['Data_' num2str(trialN) '_' num2str(2)])
    cd(code_folder)
    param_Matrix_2 = Data{2,12};
    error_2 = Data{2,8};
    cd(data_folder)
    load(['Data_' num2str(trialN) '_' num2str(3)])
    cd(code_folder)
    param_Matrix_3 = Data{2,12};
    error_3 = Data{2,8};
    cd(data_folder)
    load(['Data_' num2str(trialN) '_' num2str(4)])
    cd(code_folder)
    param_Matrix_4 = Data{2,12};
    error_4 = Data{2,8};
    ...
    L0_vec = [0.8*ones(1,1);0.9*ones(1,1);1.1*ones(1,1);1.2*ones(1,1)];
    L0_vec(index) = [];
    L0_vec = [L0_vec;1];
