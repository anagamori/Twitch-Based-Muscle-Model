...
for i = 1:3
    if i == 1
        ...
        cd(data_folder)
        load(['Data_' num2str(2) '_' num2str(1)])
        cd(code_folder)      
        color_code = [0 0 0];       
    elseif i == 2
    ...
    %%
    [pxx_spike,~] = pwelch(spike_train(5*Fs+1:end)-mean(spike_train(5*Fs+1:end)),gausswin(5*Fs),0.9*5*Fs,0:0.1:100,Fs);
    pxx_spike_norm = pxx_spike./sum(pxx_spike);
    [pxx_unit_force,~] = pwelch(force(5*Fs+1:end)-mean(force(5*Fs+1:end)),gausswin(5*Fs),0.9*5*Fs,0:0.1:100,Fs);
    [pxx_force,~] = pwelch(Force(5*Fs+1:end)-mean(Force(5*Fs+1:end)),gausswin(5*Fs),0.9*5*Fs,0:0.1:100,Fs);
