...
% Last update: 3/5/19
%==========================================================================
function [output] = spikeDrivenMuscleModel_addedNoise(Fs,time,input,modelParameter,figOpt)
...
noise_Lce = zeros(N_MU,1);
h = 1/Fs;
%% Simulation
rng('shuffle')
for t = 1:length(time)
    noise_Lce = noise(noise_Lce,100,Fs);
    if t > 1
    ...
    L_ce = L_ce + noise_Lce;
    FL(1:index_slow) = FL_slow_function(L_ce(1:index_slow));
    FL(index_slow+1:end) = FL_fast_function(L_ce(index_slow+1:end));
    
    if V_ce > 0
        FV(1:index_slow) = FVecc_slow_function(L_ce(1:index_slow),V_ce);
        FV(index_slow+1:end) = FVecc_fast_function(L_ce(index_slow+1:end),V_ce);
    else
        FV(1:index_slow) = FVcon_slow_function(L_ce(1:index_slow),V_ce);
        FV(index_slow+1:end) = FVcon_fast_function(L_ce(index_slow+1:end),V_ce);
    end
    ...
    F_ce(t) = sum(f_i);
    F_total(t) = F_ce(t) + mean(F_pe1)*F0;
